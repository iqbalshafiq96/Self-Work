...
def load_csv_data(url):
    """Loads CSV data from GitHub, automatically handling URL conversion,

    stripping UTF-8 BOM characters (\ufeff), and auto-detecting delimiters.
    """
    ...
    # utf-8-sig removes the BOM (\ufeff) prefix from column headers
    df = pd.read_csv(url, sep=None, engine="python", encoding="utf-8-sig")

    # Clean residual whitespace from column names
...
phase_selection = st.sidebar.radio(
    ...
    [
        "Phase 1: Offline Modelling & Training",
        "Phase 2: Online Monitoring & Fault Detection",
    ],
)
...
if st.session_state.phase1_completed:
    ...
    st.sidebar.caption(f"Selected PCs (A): {st.session_state.A}")
    st.sidebar.caption(
        f"Control Limit (α=0.01): {st.session_state.t2_control_limit:.2f}"
    )
    st.sidebar.caption(
        f"Warning Limit (α=0.05): {st.session_state.t2_warning_limit:.2f}"
    )
else:
...
if phase_selection == "Phase 1: Offline Modelling & Training":
    ...
    try:
        ...
        # 1. Normalization & Saving Baseline Scaling Parameters
        ...
        eigenvector_df = pd.DataFrame(
            sorted_eigenvectors, index=numeric_df.columns, columns=pc_names
        )

        diag_eigen_matrix = np.full(
            (len(sorted_eigenvalues), len(sorted_eigenvalues)), np.nan
        )
        np.fill_diagonal(diag_eigen_matrix, sorted_eigenvalues)
        eigenvalue_matrix_df = pd.DataFrame(
            diag_eigen_matrix, index=pc_names, columns=pc_names
        )

        # 4. Variance Selection Strategy (<80%)
        ...
        bar_colors = [
            "#EF553B" if val < 0.80 else "#636EFA" for val in cum_var_explained
        ]

        # 5. Hotelling T^2 Baseline & Limits Calculation
        if num_components_selected > 0:
            pc_scores_array = np.dot(
                normalized_df.values, selected_eigenvector_df.values
            )
            ...
            if n > A:
                ...
                # Save Phase 1 parameters into session state for Phase 2
                ...
                multiplier = f_val_05 = f_val_01 = t2_warning_limit = (
                    t2_control_limit
                ) = np.nan
        else:
            pc_scores_df = t2_summary_df = pd.DataFrame()
            t2_warning_limit = t2_control_limit = multiplier = df1 = df2 = (
                n
            ) = A = 0
        ...
        with tab3:
            st.subheader(
                "Correlation Matrix & Bivariate Parameter Inspection"
            )
            ...
            with c2:
                ...
                px_var = st.selectbox(
                    "X Parameter", cols_list, index=0, key="p1_x"
                )
                py_var = st.selectbox(
                    "Y Parameter",
                    cols_list,
                    index=1 if len(cols_list) > 1 else 0,
                    key="p1_y",
                )
        ...
        with tab4:
            st.subheader(
                "Eigenvalue Matrix & Cumulative Variance Analysis"
            )
            c1, c2 = st.columns([1.4, 1])
            with c1:
                fig_ev = px.imshow(
        ...
        with tab5:
            st.subheader(
                f"Selected Eigenvector Matrix ({num_components_selected} PCs <80% Cumulative Variance)"
            )
        ...
        with tab6:
            ...
            st.markdown(
                "**PC Scores = Normalized Data * Selected Eigenvectors**"
            )
        ...
        with tab7:
            ...
            st.markdown(
                "**T² Limit Formula:** T²_alpha = [ A * (n - 1) / (n - A) ] * F(A, n - A, alpha)"
            )
            c1, c2 = st.columns([1.2, 1.2])
            with c1:
                st.dataframe(
                    t2_summary_df.style.background_gradient(
                        subset=["Hotelling_T2"], cmap="YlOrRd"
                    ).format("{:.4f}"),
                    use_container_width=True,
                )
                st.write(
                    f"**Warning Limit (alpha = 0.05):** `{t2_warning_limit:.4f}`"
                )
                st.write(
                    f"**Control Limit (alpha = 0.01):** `{t2_control_limit:.4f}`"
                )
            with c2:
                fig_t2 = go.Figure()
                fig_t2.add_trace(
        ...
        st.error(f"Phase 1 Execution Error: {e}")
...
# ==============================================================================
    ...
    if not st.session_state.phase1_completed:
        st.info(
            "Phase 1 baseline model parameters not found. Please complete **Phase 1: Offline Modelling & Training** first."
        )
    else:
        try:
            ...
            # Separate Timestamp column from numerical process features
            if "Timestamp" in phase2_raw_df.columns:
                ...
                numeric_phase2_df = phase2_raw_df.select_dtypes(
                    include=[np.number]
                )

            # Re-align columns to match Phase 1 exactly
            feature_cols = st.session_state.feature_columns
            numeric_phase2_df = numeric_phase2_df[feature_cols]

            # 1. Normalize Phase 2 Data using Phase 1 Mean and Scale
            phase2_norm_array = (
                numeric_phase2_df.values - st.session_state.phase1_mean
            ) / st.session_state.phase1_scale
            phase2_norm_df = pd.DataFrame(
                phase2_norm_array,
                columns=feature_cols,
                index=phase2_raw_df.index,
            )

            # 2. Compute PC Scores using Phase 1 Selected Eigenvector Matrix
            selected_eigenvectors = st.session_state.selected_eigenvectors
            selected_eigenvalues = st.session_state.selected_eigenvalues

            phase2_scores_array = np.dot(
                phase2_norm_df.values, selected_eigenvectors.values
            )
            ...
            # 3. Compute Hotelling T^2 Statistics for Phase 2 Data
            p2_t2_components = (
                phase2_scores_array**2
            ...
            # Retrieve Control and Warning Limits set during Phase 1
            t2_warning_limit = st.session_state.t2_warning_limit
            t2_control_limit = st.session_state.t2_control_limit

            # Status Flags Assignment
            p2_t2_summary_df["Status"] = "Normal"
            p2_t2_summary_df.loc[
                p2_t2_summary_df["Hotelling_T2"] > t2_warning_limit, "Status"
            ] = "Warning"
            p2_t2_summary_df.loc[
                p2_t2_summary_df["Hotelling_T2"] > t2_control_limit, "Status"
            ] = "Fault Detected"
            ...
            with tab2_1:
                st.subheader(
                    "Phase 2 Normalized Data (Scaled via Phase 1 Parameters)"
                )
                st.caption(
                    "Standardized using Phase 1 Mean Vector (μ) and Standard Deviation Vector (σ)."
                )
            ...
            with tab2_2:
                ...
                st.markdown(
                    "**Online PC Scores = Phase 2 Normalized Case Data * Selected Eigenvectors (Phase 1)**"
                )
                st.dataframe(phase2_scores_df, use_container_width=True)

            with tab2_3:
                st.subheader(
                    "Online Hotelling T² Calculation & Fault Status"
                )
                ...
                st.dataframe(
                    p2_t2_summary_df.style.applymap(
                        highlight_status, subset=["Status"]
                    ).format({"Hotelling_T2": "{:.4f}"}),
                    use_container_width=True,
                )

            with tab2_4:
                st.subheader(
                    "Real-Time Process Monitoring & Control Chart"
                )

                x_axis = (
                    timestamp_col
                    if "Timestamp" in phase2_raw_df.columns
                    else p2_t2_summary_df.index
                )

                fig_online_t2 = go.Figure()

                # Hotelling T2 Dynamic Score Line
                fig_online_t2.add_trace(
                    go.Scatter(
                        x=x_axis,
                ...
                # Warning Limit Threshold (Phase 1)
                ...
                fig_online_t2.add_annotation(
                    x=x_axis.iloc[-1],
                    y=t2_warning_limit,
                    text=f"Phase 1 Warning Limit: {t2_warning_limit:.2f}",
                    showarrow=False,
                    yshift=10,
                    font=dict(color="Orange"),
                )

                # Control Limit Threshold (Phase 1)
                fig_online_t2.add_shape(
                    type="line",
                    x0=x_axis.iloc[0],
                    y0=t2_control_limit,
                ...
                fig_online_t2.add_annotation(
                    x=x_axis.iloc[-1],
                    y=t2_control_limit,
                    text=f"Phase 1 Control Limit: {t2_control_limit:.2f}",
                    showarrow=False,
                    yshift=10,
                    font=dict(color="Red"),
                )

                # Highlight Fault Highlights
                faults = p2_t2_summary_df[
                    p2_t2_summary_df["Status"] == "Fault Detected"
                ]
                if not faults.empty:
                    fault_x = (
                        timestamp_col.iloc[faults.index]
                        if "Timestamp" in phase2_raw_df.columns
                        else faults.index
                    )
                    fig_online_t2.add_trace(
