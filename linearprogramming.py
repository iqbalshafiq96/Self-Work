...
def page_custom():
    ...
    with st.expander("✨ Auto-parse problem statement using Google AI Studio (Gemini)", expanded=True):
        natural_prompt = st.text_area(
            ...
            placeholder="A crude distillation column splits incoming crude oil into fractions by boiling range, and two of the boundaries between adjacent products are "swing cuts" whose draw point can be adjusted: a 3,000 barrel-per-day swing cut sitting between Heavy Naphtha and Kerosene, and a 4,000 barrel-per-day swing cut sitting between Diesel and Atmospheric Gas Oil (AGO); let fracKerosene be the fraction of the first swing cut routed to Kerosene rather than Heavy Naphtha, and fracAGO be the fraction of the second swing cut routed to AGO rather than Diesel, both fractions constrained between 0 and 1, and the goal is to choose fracKerosene and fracAGO to maximize the incremental daily margin in thousands of dollars per day, given by 12 times fracKerosene minus 12 times fracAGO, since Kerosene sells at a $4-per-barrel premium over Heavy Naphtha while Diesel sells at a $3-per-barrel premium over AGO, subject to the requirement that 6 times fracKerosene minus 8 times fracAGO not exceed 2.5 to respect the downstream hydrotreater's throughput capacity, that 4 times fracKerosene minus 3 times fracAGO not exceed 2.6 to respect the column's reboiler and duty capacity, that fracKerosene not exceed 0.75 to keep the Kerosene product within its freeze-point specification, and that fracAGO be at least 0.20 to meet a contracted minimum production requirement for AGO.",
            height=100
        )
