...
def load_and_preprocess_smr_data(url_or_path):
    ...
    # Standardize Outputs (Optional, but helps PyTorch convergence)
...
try:
    df_raw, input_names, output_names, X_norm, Y_norm, scaler_X, scaler_Y = load_and_preprocess_smr_data(GITHUB_CSV_URL)
    st.sidebar.success("SMR Data Loaded & Normalized from GitHub!")
except Exception as e:
    st.error(f"Failed to load dataset: {e}. Please ensure SMR_Data.csv exists in the repo directory.")
    st.stop()
...
num_inputs = st.sidebar.number_input("Number of Inputs", min_value=1, max_value=10, value=len(input_names))
hidden1_size = st.sidebar.slider("Layer 1 Neurons", 1, 50, 12)
hidden2_size = st.sidebar.slider("Layer 2 Neurons", 0, 50, 6)  # 0 means skip
num_outputs = st.sidebar.number_input("Number of Outputs", min_value=1, max_value=10, value=len(output_names))
...
def render_pyvis_network(in_dim, h1, h2, out_dim, act_fn, initial_scale=1.5):
    ...
    {{
      "nodes": {{
        "borderWidth": 2, "size": 42,
        "font": {{ "size": 18, "face": "Source Sans Pro", "color": "#FFFFFF", "bold": true }}
      }},
      "edges": {{
        "color": {{ "color": "rgba(200, 200, 200, 0.35)", "highlight": "#3498DB" }},
        "smooth": false, "arrows": {{ "to": {{ "enabled": true, "scaleFactor": 0.5 }} }}
      }},
      "interaction": {{ "zoomView": false, "dragView": true }},
      "physics": {{ "barnesHut": {{ "gravitationalConstant": -4500, "springLength": 140, "springConstant": 0.03 }}, "minVelocity": 0.75 }}
    }}
    """)
    ...
    for i, nid in enumerate(input_nodes):
        ...
        net.add_node(nid, label=label_text, x=-450, y=(i - (in_dim - 1) / 2) * y_gap, color={"background": "#2C3E50", "border": "#5D6D7E"}, shape="circle")

    for i, nid in enumerate(h1_nodes):
        net.add_node(nid, label=f"H1 [{act_fn}]\nNode {i+1}" if h1 <= 3 else f"N{i+1}", x=-150 if h2 > 0 else 0, y=(i - (h1 - 1) / 2) * y_gap, color={"background": "#1B4F72", "border": "#3498DB"}, shape="circle")

    for i, nid in enumerate(h2_nodes):
        net.add_node(nid, label=f"H2 [{act_fn}]\nNode {i+1}" if h2 <= 3 else f"N{i+1}", x=150, y=(i - (h2 - 1) / 2) * y_gap, color={"background": "#0E6251", "border": "#1ABC9C"}, shape="circle")

    for i, nid in enumerate(output_nodes):
        label_text = f"Output\n{output_names[i]}" if i < len(output_names) else f"Output\nN{i+1}"
        net.add_node(nid, label=label_text, x=450, y=(i - (out_dim - 1) / 2) * y_gap, color={"background": "#7E5109", "border": "#F39C12"}, shape="circle")

    for src in input_nodes:
        for dst in h1_nodes: net.add_edge(src, dst)

    if h2 > 0:
        for src in h1_nodes:
            for dst in h2_nodes: net.add_edge(src, dst)
        for src in h2_nodes:
            for dst in output_nodes: net.add_edge(src, dst)
    else:
        for src in h1_nodes:
            for dst in output_nodes: net.add_edge(src, dst)
...
class ConfigurableNet(nn.Module):
    def __init__(self, in_dim, h1, h2, act_fn_name, out_dim):
        super().__init__()
        act_map = {"Tanh (tansig)": nn.Tanh(), "Sigmoid (logsig)": nn.Sigmoid(), "ReLU": nn.ReLU()}
...
# =====================================================================
st.divider()
tab_corr, tab1, tab2, tab3 = st.tabs(["Data Correlation Matrix", "Batch Training Phase", "Online Adaptation Phase", "Model Testing & Verification"])
...
with tab1:
    st.markdown("Train the model parameters using normalized SMR training inputs (`X_train`, `Y_train`).")
    ...
    if st.button("Run Batch Training"):
        if st.session_state.net is None:
            ...
            optimizer = optim.Adam(net.parameters(), lr=lr) if optimizer_choice == "Adam" else optim.SGD(net.parameters(), lr=lr)
...
with tab2:
    st.markdown("Update model weights step-by-step for incoming streaming process data.")
    stream_size = st.number_input("Streaming Samples", min_value=1, max_value=30, value=5)
...
with tab3:
    st.markdown("Evaluate actual vs. predicted performance across all **4 Output Variables** (Inverted back to engineering units).")
    ...
    if st.button("Evaluate Model on Test Set"):
        if st.session_state.net is None:
            ...
            # Plot individual curves for each of the 4 SMR Outputs
            cols = st.columns(2)
            for idx, col_name in enumerate(output_names[:4]):
                with cols[idx % 2]:
                    ...
                    y_p = Y_test_pred[:, idx]
                    r2 = 1 - (np.sum((y_t - y_p) ** 2) / (np.sum((y_t - np.mean(y_t)) ** 2) + 1e-8))
